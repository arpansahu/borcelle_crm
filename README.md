...
CMD bash -c "printenv && which python && which daphne && which celery && python manage.py collectstatic --noinput && daphne borcelle_crm.asgi:application -b 0.0.0.0 --port 8014 & celery -A borcelle_crm.celery worker -l info & celery -A borcelle_crm beat -l INFO & wait"
```
            ...
            steps {
                script {
                    ...
                    def buildInfoJson = sh(script: """
                        curl -u arpansahu:Kesar302@jenkins ${api_url}
...
Make sure to use Pipeline project and name it whatever you want I have named it as per borcelle_crm
...
Make sure to use Pipeline project and name it whatever you want I have named it as borcelle_crm_build
...
If image is pushed successfully, email is sent to notify and then another Jenkins Pipeline borcelle_crm_build is called.
